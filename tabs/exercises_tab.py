...
class ExercisesTab(ttk.Frame):
    def __init__(self, parent):
        ...
        # Tag clear button
        ttk.Button(frame, text="Clear", command=self._on_clear_search).pack(side="left", padx=5)
        ...
        # TagBox scrollbar
        tag_scrolly = tk.Scrollbar(main_frame, orient="vertical", command=self.tag_listbox.yview)
        tag_scrolly.pack(side="left", fill="y")
        self.tag_listbox.config(yscrollcommand=tag_scrolly.set)
        ...
        exercise_scrolly.pack(side="left", fill="y")
    ...
    def _on_clear_search(self, *args):
        self._setup_placeholder()
        self.selected_tag = None
        self._update_exercise_list()
    ...
    def _on_exercise_clicked(self, event):
    ...
    # ---------- CREATE COLLAPSIBLE ----------
    def create_collapsible_sections(self, ex=None, parent=None):
        """
        Create collapsible metric sections (weight, reps, sets, time, distance)
        in the given parent frame.
        """
        ...
        for col, (name, fields, units) in enumerate(sections):
            # Get data from existing exercise if provided
            data = ex.get(name, {}) if ex else {}
            has_data = bool(data.get("has", False))
            default_val = str(data.get("default", 0))
            goal_val = str(data.get("goal", 0))
            unit_val = data.get("unit", units[0] if units else None)

            # IntVar controls visibility (checked = visible)
            var = tk.IntVar(value=1 if has_data else 0)

            # Toggle checkbox
            chk = ttk.Checkbutton(checkbox_frame, text=name.capitalize(), variable=var)
            chk.grid(row=0, column=col, padx=5, sticky="w")

            # Frame that holds the metric’s entries
            ...
            # Toggle show/hide
            def toggle(v=var, f=section_frame):
                if v.get():
                    f.pack(fill="x", padx=20, pady=2)
                else:
                    f.pack_forget()

            var.trace_add("write", lambda *_, v=var, f=section_frame: toggle(v, f))

            # Create entry widgets for Default / Goal
            entries = {}
            for i, f in enumerate(fields):
                key = f.lower()
                ttk.Label(section_frame, text=f"{name.capitalize()} {f}:").grid(row=0, column=i, sticky="w", padx=5, pady=2)
                entry = ttk.Entry(section_frame, width=10)
                entry.insert(0, default_val if key == "default" else goal_val)
            ...
            if units:
                ...
                combo = ttk.Combobox(section_frame, values=units, width=8, state="readonly")
                combo.set(unit_val)
            ...
            section_widgets[name] = {"var": var, "entries": entries}
    ...
    def open_add_exercise_window(self):
        ...
        self.section_vars = self.create_collapsible_sections(parent=frame)

        # ---------- SAVE FUNCTION ----------
        def save_exercise():
            try:
                exercise = self._get_exercise_data_from_entries(self.entries)
                for name, widgets in self.section_vars.items():
                    var = widgets["var"].get()
                    entries = widgets["entries"]

                    exercise[name] = {
                        "has": bool(var),
                        "default": float(entries["default"].get() or 0),
                        "goal": float(entries["goal"].get() or 0),
                    }
                    if entries["unit"]:
                        exercise[name]["unit"] = entries["unit"].get()
        ...
        ttk.Button(button_frame, text="Save Exercise", command=save_exercise).pack(side="left", padx=10)
        ttk.Button(button_frame, text="Close", command=popup.destroy).pack(side="left", padx=10)
    ...
    def edit_exercise_popup(self, ex, parent_popup=None):
        ...
        self.section_vars = self.create_collapsible_sections(ex=ex, parent=frame)
        ...
        if confirm:
            try:
                ...
                for name, widgets in self.section_vars.items():
                    ...
                    if entries["unit"]:
                        updates[name]["unit"] = entries["unit"].get()
